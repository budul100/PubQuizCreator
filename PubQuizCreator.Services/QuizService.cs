...
namespace PubQuizCreator.Services
{
    public class QuizService(AppDbContext db)
    {
        ...
        public async Task<Quiz?> GetDetailAsync(Guid id, CancellationToken ct = default) => await db.Quizzes
        ...
        public async Task<List<Quiz>> GetPastAsync(CancellationToken ct = default) => await db.Quizzes
        ...
        public async Task<int> GetTotalOpenSlotsAsync(CancellationToken ct = default) => await db.QuizSlots
            .Where(s => s.QuestionId == null
                && s.Round.Quiz.Date >= DateOnly.FromDateTime(DateTime.Today))
            .CountAsync(ct);

        public async Task<List<Quiz>> GetUpcomingAsync(CancellationToken ct = default) => await db.Quizzes
            .Include(q => q.Rounds).ThenInclude(r => r.Slots)
            .Where(q => q.Date >= DateOnly.FromDateTime(DateTime.Today))
            .OrderBy(q => q.Date)
            .AsSplitQuery()
